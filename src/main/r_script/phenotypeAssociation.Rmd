...
loincAnnotationStat <- read.csv("./data/loincAnnotationstats_v2.0.tsv", header = TRUE, sep = "\t")
```
...
p <- loinc2hpo %>% select(mapFailed, encounter_num, patient_num, concept_cd, start_date) %>% group_by(mapFailed) %>% summarise(counts = n()) %>%
...
```{r}
ggsave("./data/images/Hush lab test to HPO conversion.png", plot = p, width = 1.2, height = 1.2)
ggsave("./data/images/Hush lab test to HPO conversion.pdf", plot = p, width = 1.2, height = 1.2)
```
...
```{r}
ggsave("/Users/zhangx/git/HushToFhir/data/images/unique HPO term counts for each patient.png", plot = p2, width = 3, height = 2)
ggsave("/Users/zhangx/git/HushToFhir/data/images/unique HPO term counts for each patient.pdf", plot = p2, width = 3, height = 2)
```
...
```{r}
ggsave("./data/images/loinc_type_aggreg.png", plot = p3, width = 1.7, height = 1.5)
ggsave("./data/images/loinc_type_aggreg.pdf", plot = p3, width = 1.7, height = 1.5)
```
...
```{r}
patientAbnormalTermMapFreq_inferred <- read.table("./data/patientAbnormalTermMapFreq-inferred_v2.0.csv", sep = "\t", header = TRUE, stringsAsFactors = FALSE) 
...
beforeInfer <- mean(abnormalTermsBeforeInferPerPatient$count_before_infer)
afterInfer <- mean(abnormalTermsWithInferPerPatient$count_after_infer)
beforeInfer
afterInfer
afterInfer - beforeInfer
```
...
ggsave("./data/images/patient_sex_age_distribution.png", plot = ageDistribution, width = 2, height = 1.5)
ggsave("./data/images/patient_sex_age_distribution.pdf", plot = ageDistribution, width = 2, height = 1.5)
```
...
vitamin abnormality analysis
```{r}
patient_phenotypes <- colnames(df4ML)
vitamin_phenotypes <- patient_phenotypes[str_detect(patient_phenotypes, "[Vv]itamin|folate|calci[(triol)|(fediol)]")]
df4ML %>% select(vitamin_phenotypes) %>% map_int(function(x){sum(x>=3)})


vitaminMatrixNoInfer <- patientTermMapFreq %>% filter(isNegated == "false" & str_detect(hpoTerm, "[Vv]itamin|folate|calci[(triol)|(fediol)]")) %>% spread(hpoTerm, hpoTermFreq, fill = 0)
patient_phenotypes <- colnames(vitaminMatrixNoInfer)
vitamin_phenotypes <- patient_phenotypes[str_detect(patient_phenotypes, "[Vv]itamin|folate|calci[(triol)|(fediol)]")]
vitaminMatrixNoInfer %>% select(vitamin_phenotypes) %>% map_int(function(x){sum(x>=3)})
```
