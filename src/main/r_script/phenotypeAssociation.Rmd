...
loincAnnotationStat <- read.csv("./data/loincAnnotationstats8-2-2018.tsv", header = TRUE, sep = "\t")
```
...
hpoTermMap <- patientTermMapFreq_inferred %>% select(hpoTerm) %>% na.omit() %>% distinct() %>% mutate(hpo = make.names(hpoTerm))
```
...
df_glm <- df_glm %>% mutate(withAcuteAsthmaICDCode = ifelse(is.na(withAcuteAsthma), "N", "Y")) %>% select(-withAcuteAsthma)
...
glm_Y = df_glm %>% select(-c(1:8, ncol(df_glm))) # select all the HPO columns
library(caret)
...
glm_p_value <- function(hpo, x = glm_X) {
  target <- as.factor(ifelse(hpo >= TERMFREQ_THRESHOLD, 1, ifelse(hpo == 0, 0, 0.5)))
  df <- cbind(x, hpo, target) %>% filter(target == 0 | target == 1) # exclude patients that had a phenotype but did not reach the threshold
  #df_severe <- df %>% filter(isFrequent == 1)
  #df_nonSevere <- df %>% filter(isFrequent == 0)
