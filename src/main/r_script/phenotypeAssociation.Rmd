...
uniqueHPOperPatientNoInfer %>% filter(isNegated == "false") %>% select(counts) %>% map2_dbl(., "mean")
...
```{r}
patientAbnormalTermMapFreq_inferred <- read.table("./data/patientAbnormalTermMapFreq-inferred.csv", sep = "\t", header = TRUE, stringsAsFactors = FALSE) %>% mutate(hpoTerm = make.names(hpoTerm), isNegated = as.factor("false")) %>% rename(hpo = hpoTerm)
