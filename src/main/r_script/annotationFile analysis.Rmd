...
noOfLOINCsMappedToEachHPO %>% filter(noLoinc == 10) %>% map_chr(~.$hpoTermId, "+")
ggplot(noOfLOINCsMappedToEachHPO) + geom_histogram(aes(x = noLoinc, y = ..count../sum(..count..)), fill = "black", binwidth = 1) + geom_label(aes(x = 10, y = 0.1, label = "example")) +
  scale_y_continuous(labels = scales::percent)+
