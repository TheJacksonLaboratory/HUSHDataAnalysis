...
registerDoSNOW(c1)
model <- train(training[,-target.index], training$isSevere, method = "bayesglm", trControl = train.ctrl, preProcess = c("BoxCox"))
stopCluster(c1)
...
```{r}
library(mboost)
train.ctrl = trainControl(method = "repeatedcv", number = 10, repeats = 1)
tune.grid = expand.grid(prune = c("yes", "no"), mstop = c(3))
target.index = ncol(training)
c1 <- makeCluster(3, type = "SOCK")
registerDoSNOW(c1)
model <- train(training[,-target.index], training$isSevere, method = "glmboost", trControl = train.ctrl, tuneGrid = tune.grid)
stopCluster(c1)
prediction <- predict(model, testing[,-target.index])
cm <- confusionMatrix(prediction, testing$isSevere)
# surpring: 100% sensitivity and specificity
...
testing$isSevere <- as.factor(testing$isSevere)
train.ctrl = trainControl(method = "repeatedcv", number = 10, repeats = 1)
tune.grid <- expand.grid(tau = c(0.001, 0.01, 0.1, 1, 5))
target.index = ncol(training)
c1 <- makeCluster(3, type = "SOCK")
registerDoSNOW(c1)
model <- train(training[,-target.index], training$isSevere, method = "lssvmLinear", trControl = train.ctrl, preProcess = c("BoxCox"), tuneGrid = tune.grid)
stopCluster(c1)
prediction <- predict(model, testing[,-target.index])
cm <- confusionMatrix(prediction, testing$isSevere)
# surpring: 100% sensitivity and specificity
...
tune.grid <- expand.grid(size=c(5, 10, 15), decay=c(0.01, 0.1, 1, 10))
target.index = ncol(training)
c1 <- makeCluster(3, type = "SOCK")
registerDoSNOW(c1)
model <- train(training[,-target.index], training$isSevere, method = "nnet", trControl = train.ctrl, preProcess = c("BoxCox"), tuneGrid = tune.grid)
stopCluster(c1)
prediction <- predict(model, testing[,-target.index])
cm <- confusionMatrix(prediction, testing$isSevere)
# surpring: 100% sensitivity and specificity
