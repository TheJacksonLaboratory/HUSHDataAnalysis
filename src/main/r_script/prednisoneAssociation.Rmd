...
glm_p_value <- function(hpo, x = glm_X) {
  ...
  glmmodel <- glm(target ~ isSevere + withAsthmaICDCode, data = df, family = binomial(link = "logit"))
...
for (i in 1:10) {
  ...
  result <- data.frame(hpo = names(tests), 
                           estimate.isSevere = tests %>% map_dbl(~.[2, 1]),
                           estimate.withAsthmaICDCode = tests %>% map_dbl(~.[3,1]),
...
finalResult %>% ggplot() +
  geom_point(aes(x= Z.isSevere, y = Z.withAsthmaICDCode))
```
