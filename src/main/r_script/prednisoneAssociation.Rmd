...
To simplify data query, we have parsed the dataset in Java and imported all records to a SQLite database. The source code can be accessed through https://github.com/TheJacksonLaboratory/HUSHDataAnalysis. 

Additionally, we select all diagnosis records (with ICD codes) and imported them to the database as a separate table.

(NOTE: java code will not be evaluated here)
```{bash eval = FALSE}
...
For Goal I, the dataset is too big and the task too complicated to implicate in R. Instead, we implicated the conversion in Java.
```{bash eval = FALSE}
...
We also counted prednisone prescription times for each patient in Java. 
```{bash eval = FALSE}
...
loinc2hpo <- read.csv("/Users/zhangx/Documents/HUSH+_UNC_JAX/Hush+UNC_JAX/HUSH+/loinc2hpotransformation_v2.0.txt", header = TRUE, sep = "\t")
...
loincAnnotationStat <- read.csv("/Users/zhangx/git/HushToFhir/data/loincAnnotationstats_v2.0.tsv", header = TRUE, sep = "\t")
hpoTermList <- read.table("./data/hpoTermList.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
hpoTermList$name = make.names(hpoTermList$label)
```
...
aggrgLOINCtypes <- data.frame(aggrgtedFrom = factor(c("1", "2", ">3"), levels = c("1", "2", ">3")), count = c(aggrgedFromOneLOINC, aggrgedFromTwoLOINC, aggrgedFromThreeAndMoreLOINC))  
  ...
  ggplot(aggrgLOINCtypes) + geom_bar(aes(x = 1, y = count, fill = aggrgtedFrom), stat = "identity", color = "black", size = 0.5) + coord_polar(theta = "y") +
...
export patient terms, use the java command line to infer parent terms (only abnormal phenotypes)
```{r}
write.csv(patientTermMapFreq, "./data/patientTermMapFreq_v2.0.csv",  row.names = FALSE, quote = FALSE)
#
# run java command line
#

patientTermMapFreq_inferred <- read.table("./data/patientAbnormalTermMapFreq-inferred_v2.0.csv", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
patientTermMapFreq_inferred <- patientTermMapFreq_inferred %>% mutate(hpoTerm = make.names(hpoTerm), isNegated = as.factor("false"))
```
...
combined <- patients %>%
  ...
  inner_join(patientTermMapFreq_inferred, by = "patient_num") %>%
...
```{r}
write.csv(df4ML, "./data/Hush+_data_matrix_v2.0.csv", quote = FALSE, row.names = FALSE)

df4ML <- read.csv("/Users/zhangx/git/HushToFhir/data/Hush+_data_matrix_v2.0.csv", header = TRUE)
```
...
glm_Y = df_glm %>% select(9:ncol(df_glm) - 1)
library(caret)
...
glm_Y = df_glm %>% select(-c(1:8, ncol(df_glm)))
library(caret)
low_variance <- nearZeroVar(glm_Y)
glm_Y <- glm_Y[,-low_variance]  
...
#l <- list(10)
  ...
  result_logistic <- result_logistic %>% 
    left_join(hpoTermList, by = c("hpo" = "name")) %>%  mutate(hpo = label) %>% select(colnames(result_logistic))
    
  significance_star <- function(x) {
    p_stars <- vector(mode = "character", length = length(x))
    for (i in 1:length(x)) {
      if (x[i] < 0.01) {
        p_stars[i] = "**"
      } else if (x[i] < 0.05) {
  ...
  result_logistic <- result_logistic %>% mutate(significance_steroid = significance_star(p_steroid)) %>% mutate(significance_asthma.diagnosis = significance_star(p_asthma.diagnosis)) %>% select(hpo, odds.ratio_steroid, CI.95.._steroid, p_steroid, significance_steroid, odds.ratio_asthma.diagnosis, CI.95.._asthma.diagnosis, p_asthma.diagnosis, significance_asthma.diagnosis)
  
  write.csv(result_logistic, "./data/logistic_regression_v2.0.csv", quote = FALSE, row.names = FALSE)
...
#finalResult %>% arrange(-estimate.withAsthmaICDCode) %>% filter(p.isSevere < 1, p.withAsthmaICDCode < 1) %>% View()
  ...
  glm_X %>% mutate(isFrequent = prscbCount >= SEVERITY_THRESHOLD) %>% group_by(withAsthmaICDCode, isFrequent) %>% summarise(total = n()) %>% ungroup() %>% spread(key = isFrequent, value = total)
...
```{r}
library(tidyverse)
df4ML <- read.csv("data/Hush+_data_matrix_v2.0.csv", header = TRUE)
...
target.index = ncol(training)
tuneGrid = expand.grid()
dtrain <- xgb.DMatrix(data = as.matrix(training[,-target.index]), label = as.numeric(training[,target.index] == "X1"))
model <- xgboost(data = dtrain, max.depth = 50, eta = 1, nthread = 4, nrounds = 50, objective = "binary:logistic")

prediction <- predict(model, as.matrix(testing[,-target.index]))
cm <- confusionMatrix(as.factor(as.numeric(prediction > 0.5)), as.factor(as.numeric(testing$isSevere == "X1")))
cm
prediction <- predict(model, as.matrix(testing[,-target.index]), type = "prob")
require(pROC)
roc1 <- roc(testing$isSevere, prediction, smooth = TRUE)
data.frame(sensitivity = roc1$sensitivities, specificity = roc1$specificities) %>% ggplot() + geom_point(aes(1-specificity, sensitivity), size = 0.5) + xlab("false positive rate") + ylab("true positive rate")
