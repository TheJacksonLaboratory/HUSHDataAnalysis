...
patient_withDiabetes <- dbGetQuery(db, "SELECT DISTINCT(patient_num) FROM ICD WHERE icd LIKE 'ICD9:250%' OR icd LIKE 'ICD10:E11%'")
patient_withDiabetes <- patient_withDiabetes %>% mutate(withDiabeteICDCode = "Y")
...
combined <- patients %>%
  ...
  left_join(patient_withDiabetes) %>% #filter out patients with diabetes
...
patientCombinedCounts <- combined %>% 
  ...
  filter(withAsthmaICDCode == "Y") %>% #ignore patients that is not diagnosed with asthma
