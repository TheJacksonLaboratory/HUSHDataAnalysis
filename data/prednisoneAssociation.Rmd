...
determine how many LOINC codes are aggregated for each HPO term
```{r}
...
```{r}
visitCount <- dbGetQuery(db, "SELECT patient_num, count(encounter_num) as visit_count FROM VISIT_DIMENSION GROUP BY patient_num ORDER BY visit_count")
```
