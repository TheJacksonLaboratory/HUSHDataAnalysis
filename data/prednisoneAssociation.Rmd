...
patient_withDiabetes <- dbGetQuery(db, "SELECT DISTINCT(patient_num) FROM ICD WHERE icd LIKE 'ICD9:250%' OR icd LIKE 'ICD10:E11%'") %>% patient_withDiabetes %>% mutate(withDiabeteICDCode = "Y")
...
patientCombinedCounts <- combined %>% 
  filter(encounter_no_days >= VISITTIMES_THRESHOLD) %>% #ignore patients that only occationally visited hospitals
  filter(withAsthmaICDCode == "N") %>% #ignore patients that is not diagnosed with asthma
  filter(withDiabeteICDCode == "N") %>% #ignore patients that have been diagnosed with diabetes
  filter(withChronicLiverDamageICDCode == "N") %>% # ignore patients that have been diagnosed with chronic liver damage
...
#   filter(encounter_no_days > VISITTIMES_THRESHOLD) %>% #ignore patients that only occationally visited hospitals
#   filter(prscbCount > SEVERITY_THRESHOLD) %>% #select severe patients
...
# patientCombinedCounts <- patientOnPrednisonCount %>% rename(severePatientNo = count) %>% 
  # full_join(patientNotOnPrednisonCount) %>% rename(notSeverePatientNo = count) %>%
  # mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>%
  # select(groupName, notSeverePatientNo, severePatientNo)
```

Calculate population distribution
```{r}
populationCount <- combined %>%
  filter(withAsthmaICDCode == "N") %>%
  filter(withDiabeteICDCode == "N") %>%
  filter(withChronicLiverDamageICDCode == "N") %>%
...
outcome <- patientCombinedCounts %>% 
  ...
  select(hpo, mappedTo = loincMappedTo, aggrgtdFrom = loincAggrgtdFrom, `notSeverePatientNo(%)`, `severePatientNo(%)`, pVal) %>% 
  arrange(pVal)

outcome_for_compute <- patientCombinedCounts %>% 
  #rename(hpo = groupName) %>%
  mutate(`notSeverePatientNo(%)` = paste(nonSevere, "[", round(100 * nonSevere/populationCount$count[1], 1), "]", sep = "")) %>%
...
```{r}
asthmaNoDiabetesNoLiverDamage <- outcome_for_compute
noasthmaNoDiabetesNoLiverDamage <- outcome_for_compute
...
joined_data %>% mutate(tolabel = ifelse(significe_asthma == "Y" & significance_no_asthma == "N", hpo, "")) %>% 
  ggplot() + geom_point(aes(x = foldchange_no_asthma, y = foldchange_asthma)) + geom_text(aes(x = foldchange_no_asthma, y = foldchange_asthma, label = tolabel))
```
