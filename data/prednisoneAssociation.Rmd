...
#combined %>% filter(isNegated == "false") %>% spread(key = hpoTerm, value = hpoTermFreq, fill = 0) %>% View()  
```
...
patientCombinedCounts <- patientCombinedCounts %>% 
  rename(hpo = groupName) %>%
  mutate(`notSeverePatientNo(%)` = paste(notSeverePatientNo, "[", round(100 * notSeverePatientNo/populationCount[[2]], 1), "]", sep = "")) %>%
  mutate(`severePatientNo(%)` = paste(severePatientNo, "[", round(100 * severePatientNo/populationCount[[3]], 1), "]", sep = "")) %>%
...
```{r}
visitCountByUniqueStartDays <- read.csv("/Users/zhangx/Documents/HUSH+_UNC_JAX/Hush+UNC_JAX/patient_visit_counts_with_no_start_days.csv", header = TRUE)
ggplot(visitCountByUniqueStartDays) + geom_histogram(aes(x = encounter_no_days), fill = "white", color = "black", binwidth = 20) + scale_x_continuous(limits = c(0, 500))
```
