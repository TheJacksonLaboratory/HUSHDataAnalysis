...
df4ML <- combined %>% filter(isNegated == "false") %>% spread(key = hpoTerm, value = hpoTermFreq, fill = 0) %>% mutate (isSevere = ifelse(prscbCount >= 3, 1, 0)) %>% select(-c(isNegated, prscbCount)) 
```
...
```{r}
populationCount <- combined %>% select(patient_num, prscbCount) %>% distinct() %>% mutate(severity = ifelse(prscbCount >= SEVERITY_THRESHOLD, "severity", "nonSevere")) %>% select(-prscbCount) %>% group_by(severity) %>% summarise(count = n())
...
```{r}
write.csv(df4ML, "Hush+_data_matrix.csv")
write.csv(outcome, "patientCombinedCounts.csv")
```
